    ...
    if 'Login key has already been sent.' in data:
        while True :
            if 'Invalid login key.' in data:
                platformtools.dialog_ok('openload', 'El codigo introducido no es valido revisa tu correo e introduce el codigo correcto')

            code = platformtools.dialog_input(  post.get('LoginForm[loginkey]', ''),
                                                "Introduzca el codigo que ha sido enviado a \'%s\'" % 'r_dav'
                                              )
            if not code:
                break
            else:
...
#MP4Upload
    ...
    actualizados = args['direccion'][0]
    data = read(actualizados)
    pattern =  '(\<iframe class="metaframe rptss".*</iframe>)'
    matches = re.findall(pattern,data,re.IGNORECASE)


    for match in matches:
        pattern = 'src="(.*?)"'
        zelda = re.findall(pattern,match,re.MULTILINE)[0]



        # zelda = 'https://openload.co/embed/'+ matches[1]
        oload = get_ol_video_url(zelda)
        url = build_url({'mode': 'play', 'playlink': oload })
        li = xbmcgui.ListItem('[COLOR skyblue][B]Opcion Openload[/B][/COLOR]', iconImage='https://i1.wp.com/www.gamerfocus.co/wp-content/uploads/2017/03/anime.jpeg',
